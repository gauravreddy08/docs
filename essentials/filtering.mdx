...
Metadata filtering allows you to narrow down search results based on specific criteria before or after vector similarity ranking. This enables precise queries like "find similar documents that are published, in English, and have a rating above 4.0."

<Info>
Filters work with all index types (Linear, IVF, NSW) and can dramatically improve search relevance by combining semantic similarity with exact metadata matches.
</Info>
...
<Tabs>
  ...
  <Tab title="IVF Index">
    **Filters applied to candidate clusters**
    
    ```json
    // More selective filters reduce cluster search time
    {
      "filters": {
    ...
    Highly selective filters can significantly speed up search.
  </Tab>
  
  <Tab title="NSW Index">
    **Filters applied during graph traversal**
    
    ```json
    // Boolean filters work best during graph navigation
    {
      "filters": {
        "published": true,               // Fast boolean check
        "category": {"in": ["a", "b"]}   // Fast membership test
      }
    }
    ```
    
    Simple equality and boolean filters are most efficient.
  </Tab>
</Tabs>
